...
6. デスクトップのWinSCPアイコンをダブルクリックして起動する。
7. ホスト名: 192.168.9.100, ユーザ名: \<username\> を入力し、セッション名をつけて保存する。

## DBeaver のインストールと設定 ※必要に応じて実行
...
4. 最後に「インストール」をクリックしてインストール。
5. インストール完了画面が表示されたら、「Finish」をクリックしてインストーラーを終了する。
6. スタートメニューやデスクトップのショートカットなどから DBeaver を起動する。
7. 上部のタブから データベース > 新しい接続 を選択する。
8. 「新しい接続タイプを選択する」ウインドウが開くので、SQL > MariaDB を選択して「次へ」をクリックする。
9. Server Host: 192.168.9.100, Port:3306, Database: gxd, ユーザー名: gxd_pipeline, パスワード: gw!2341234 を入力し「終了」をクリックする。
10. 「データベースナビゲータ」タブに gxd が表示されていればOK.
